...
let saveExerciseBtn = document.getElementById('saveExercise')
let exerciseNameInputWrapper = document.getElementById('exerciseNameInputWrapper')
let exerciseNameInput = document.getElementById('exerciseNameInput')
let exerciseNameDoneBtn = document.getElementById('exerciseNameDoneBtn')
let newExerciseInputWrapper = document.getElementById('newExerciseInputWrapper')
let exercisContainer = document.getElementById('exerciseContainer')
let weightInput = document.getElementsByClassName('weight')
let repInput = document.getElementsByClassName('rep')
...
var Controller = {
  addExercise: function(nameOfExercise){
    var exerciseId = uuid()
    var newExercise = {
...
function getWorkoutsFromLocalStorage(){
  let noHistoryParagraph = document.querySelector('.noHistory')
  workoutHistory = []
  for(var i = 0; i < localStorage.length; i++){
    if(localStorage.length > 0){
    workoutHistory.push(JSON.parse(localStorage.getItem('workout' + i.toString())))
      }else{
        console.log('there are no workouts')
        noHistoryParagraph.className = 'noHistory visible'
    }
  }
  return
}
...
// ------------- Things to do -------------
// BUG - checkboxes go away after a new set has been populated
// FEATURE
// 1# - create a workout history section - See scratchPad.js ( Continue )
...
// make a re-usable function to get to the specific exercise or set because i keep writing for loops a lot.
